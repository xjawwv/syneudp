...
export default function InstanceDetailPage() {
  ...
  const cleanHost = instance.host.replace(/^https?:\/\//, "");
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <button
                  onClick={() => copyToClipboard(cleanHost)}
            ...
            <div>
              <label className="text-sm text-gray-500">Connection URI</label>
              <div className="flex items-center gap-2 mt-1">
                <code className="flex-1 bg-gray-100 px-3 py-2 rounded text-sm break-all">
                  {instance.engine === "mongodb" 
                    ? `mongodb://${instance.dbUser}:${showPassword ? instance.password : "••••••••••••"}@${cleanHost}:${instance.port}/${instance.dbName}?authSource=admin`
                    : `${instance.engine}://${instance.dbUser}:${showPassword ? instance.password : "••••••••••••"}@${cleanHost}:${instance.port}/${instance.dbName}`}
                </code>
                <button
                  onClick={() => {
                    const uri = instance.engine === "mongodb" 
                      ? `mongodb://${instance.dbUser}:${instance.password}@${cleanHost}:${instance.port}/${instance.dbName}?authSource=admin`
                      : `${instance.engine}://${instance.dbUser}:${instance.password}@${cleanHost}:${instance.port}/${instance.dbName}`;
                    copyToClipboard(uri);
                  }}
                  className="text-gray-400 hover:text-gray-600"
                >
